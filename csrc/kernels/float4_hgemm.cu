...
template<int Bm = 128, int Bn = 128, int Bk = 8, int blockSize = 256, int A_BLOCK_DIM_X = 8,
         int B_BLOCK_DIM_X = 32, int C_BLOCK_DIM_X = 16, int C_WARP_DIM_X = 8, int C_WARP_DIM_Y = 4>
__global__ void float4_hgemm_kernel(
    ...
    const int WARP_SIZE = C_WARP_DIM_X * C_WARP_DIM_Y;

    int warp_id = tid / WARP_SIZE;
    int lane_id = tid % WARP_SIZE;

    constexpr int C_WARP_NUM_X = C_BLOCK_DIM_X / C_WARP_DIM_X;

    int warp_X = warp_id % C_WARP_NUM_X;
    int warp_Y = warp_id / C_WARP_NUM_X;

    int lane_Y = lane_id / C_WARP_DIM_X;
    int lane_X = lane_id % C_WARP_DIM_X;

    int c_tid_y = warp_Y * C_WARP_DIM_Y + lane_Y;
    int c_tid_x = warp_X * C_WARP_DIM_X + lane_X;
...
#pragma unroll
            for(int j = a_tid_x; j < Bk; j += A_BLOCK_DIM_X){
                int tx = k + j;
                As[j][i] = (ty < M && tx < K) ? A[ty * K + tx] : __float2half(0.0f);
            }
        }
...
#pragma unroll
            for (int i = 0; i < Tm / 4; ++i) {
                int r = (c_tid_y + i * C_BLOCK_DIM_Y) * 4;
                FLOAT4(regA[i * 4]) = FLOAT4(As[p][r]);
            }
#pragma unroll
            for (int i = 0; i < Tn / 4; ++i) {
                int c = (c_tid_x + i * C_BLOCK_DIM_X) * 4;
...
#pragma unroll
    ...
    for(int i = 0; i < Tm; ++i){
        int ty = tileC_y0 + c_tid_y * 4 + i / 4 * 4 * C_BLOCK_DIM_Y + i % 4;
        for(int j = 0; j < Tn; ++j){
            int tx = tileC_x0 + c_tid_x * 4 + j / 4 * 4 * C_BLOCK_DIM_X + j % 4;
            if (ty < M && tx < N) { C[ty * N + tx] = __float2half(Ct[i][j]); }
        }
    }
}

torch::Tensor float4_hgemm(torch::Tensor A, torch::Tensor B) {
    CHECK_CUDA(A);
    CHECK_CUDA(B);
    CHECK_FP16(A);
    CHECK_FP16(B);
    ...
    const int C_WARP_DIM_X = 8, C_WARP_DIM_Y = 4;
    ...
    float4_hgemm_kernel<
        Bm, Bn, Bk, blockSize, A_BLOCK_DIM_X, B_BLOCK_DIM_X, C_BLOCK_DIM_X, C_WARP_DIM_X, C_WARP_DIM_Y
    ><<<grid, block>>>(
        A_ptr, B_ptr, C_ptr,
        M, K, N
    );
